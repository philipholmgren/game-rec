...
import type { CustomMoodsModalProps } from '../types/componentPropTypes';

export default function CustomMoodsModal({
  isOpen,
  onClose,
  customMoods,
  setCustomMoods,
}: CustomMoodsModalProps) {
  ...
  const handleRemoveMood = (moodToRemove: string) => {
    setCustomMoods((prev) => prev.filter((mood) => mood !== moodToRemove));
  };
